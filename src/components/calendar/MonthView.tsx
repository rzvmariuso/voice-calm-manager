...
export function MonthView({ 
  ...
  const [panelSizes, setPanelSizes] = useState(() => {
    const saved = localStorage.getItem('calendar-month-panels');
    return saved ? JSON.parse(saved) : Array(7).fill(100 / 7);
  });

  useEffect(() => {
    localStorage.setItem('calendar-month-panels', JSON.stringify(panelSizes));
  }, [panelSizes]);
  ...
  return (
    <ResizablePanelGroup direction="vertical" className="min-h-[600px]">
      <ResizablePanel defaultSize={20} minSize={15} maxSize={30}>
        <div className="bg-background rounded-t-lg border border-b-0 shadow-soft">
          {/* Calendar Header */}
          <div className="border-b border-border p-4">
            <ResizablePanelGroup direction="horizontal" className="min-h-[60px]" onLayout={setPanelSizes}>
              {['Montag', 'Dienstag', 'Mittwoch', 'Donnerstag', 'Freitag', 'Samstag', 'Sonntag'].map((day, index) => (
                <React.Fragment key={day}>
                  <ResizablePanel defaultSize={panelSizes[index]} minSize={8} maxSize={25}>
      ...
      <ResizablePanel defaultSize={80} minSize={60}>
        <div className="bg-background rounded-b-lg border border-t-0 shadow-soft">
          {/* Calendar Grid */}
                ...
                {week.map((date, dayIndex) => {
                  ...
                  return (
                    <React.Fragment key={dayIndex}>
                      <ResizablePanel defaultSize={panelSizes[dayIndex]} minSize={8} maxSize={25}>
                        <div
                          className={cn(
